"""Independence checks for PMSM analytical reference validation."""
...
from reference_case_builder import FIXTURE_PATH, build_independent_reference_values, load_pmsm_reference_cases

from helpers import build_sample_legacy_params
import reference_case_builder
...
def test_reference_case_builder_does_not_import_production_modules():
    source = inspect.getsource(reference_case_builder)

    assert "motor_core" not in source
    assert "pmsm_ke_kt_models" not in source
    assert "electrical_semantics" not in source
    assert "calculations" not in source
    assert "calculate_revised_" not in source
    assert "derive_revised_" not in source


def test_reference_fixture_expected_values_are_rebuilt_from_fixture_primitives():
    for case in load_pmsm_reference_cases():
...
def test_legacy_baseline_fixture_hash_is_unchanged():
    ...
    assert FIXTURE_PATH.name == "pmsm_reference_cases.json"


def test_pmsm_reference_validation_keeps_legacy_downstream_defaults():
